...
// MARK: - Data Model
//
struct ClubSample: CreatableFromJSON { // TODO: Rename this struct
    let clubName: String
    let headquarters: String
    let members: [Members]
    init(clubName: String, headquarters: String, members: [Members]) {
        self.clubName = clubName
        self.headquarters = headquarters
        self.members = members
    }
    init?(json: [String: Any]) {
        guard let clubName = json["club-name"] as? String else { return nil }
        guard let headquarters = json["headquarters"] as? String else { return nil }
        guard let members = Members.createRequiredInstances(from: json, arrayKey: "members") else { return nil }
        self.init(clubName: clubName, headquarters: headquarters, members: members)
    }
    struct Members: CreatableFromJSON { // TODO: Rename this struct
        let birthday: Date
        let favoriteColor: String?
        let firstName: String
        let hobbies: [Hobbies]
        let inchesTall: Double
        let lastName: String
        let websites: [URL]
        init(birthday: Date, favoriteColor: String?, firstName: String, hobbies: [Hobbies], inchesTall: Double, lastName: String, websites: [URL]) {
            self.birthday = birthday
            self.favoriteColor = favoriteColor
            self.firstName = firstName
            self.hobbies = hobbies
            self.inchesTall = inchesTall
            self.lastName = lastName
            self.websites = websites
        }
        init?(json: [String: Any]) {
            guard let birthday = Date(json: json, key: "birthday", format: "yyyy-MM-dd") else { return nil }
            guard let firstName = json["first-name"] as? String else { return nil }
            guard let hobbies = Hobbies.createRequiredInstances(from: json, arrayKey: "hobbies") else { return nil }
            guard let inchesTall = Double(json: json, key: "inches-tall") else { return nil }
            guard let lastName = json["last-name"] as? String else { return nil }
            guard let websites = (json["websites"] as? [String]).flatMap({ $0.toURLArray() }) else { return nil }
            let favoriteColor = json["favorite-color"] as? String
            self.init(birthday: birthday, favoriteColor: favoriteColor, firstName: firstName, hobbies: hobbies, inchesTall: inchesTall, lastName: lastName, websites: websites)
        }
        struct Hobbies: CreatableFromJSON { // TODO: Rename this struct
            let name: String
            let type: String
